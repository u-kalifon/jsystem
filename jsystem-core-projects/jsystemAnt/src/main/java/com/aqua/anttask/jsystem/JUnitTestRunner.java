...
import junit.framework.JUnit4TestAdapterForJSystem;
...
public class JUnitTestRunner implements TestListener, JUnitTaskMirror.JUnitTestRunnerMirror {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    try {
                    	int lastDotIndex = junitTest.getName().lastIndexOf('.');
                    	String className = junitTest.getName().substring(0, lastDotIndex);
                    	methodName = junitTest.getName().substring(lastDotIndex+1);
	                    if (loader == null) {
                ...
                if(methodName != null){
                	...
                	if (testClass != null && TestCase.class.isAssignableFrom(testClass)) {
                		...
                		// Method from a JUnit4 test case (Gooli)
                		suite = new JUnit4TestAdapterForJSystem(testClass, methodName);
                		System.setProperty(RunningProperties.METHOD_NAME, methodName);
                	}
                } else {
    ...
    private void logTestListenerEvent(String msg) {
        ...
        if (logTestListenerEvents) {
            out.flush();
            out.println(JSystemTask.TESTLISTENER_PREFIX + msg);
            out.flush();
        }
    }
    ...
    public void handleOutput(String output) {
        if (!logTestListenerEvents && output.startsWith(JSystemTask.TESTLISTENER_PREFIX)) {
            // ignore
