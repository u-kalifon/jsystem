 ...
 * See the doSequentialIteration method.
 * 
...
public class ForTask extends Task {
	...
	/**
	 * Set the keepgoing attribute, indicating whether we should stop on errors
	 * or continue heedlessly onward.
	 * 
	 * @param keepgoing
	 ...
	 * 
	 * @param delimiter
	 *            the delimiter used to separate the tokens in the list
	...
	/**
	 * This is a path that can be used instread of the list attribute to
	 * interate over. If this is set, each path element in the path is used for
	 * an interator of the sequential element.
	 * 
	 * @param path
	 ...
	 * there are ok, it calls doTheTasks() which constructes a macrodef task and
	 * a for each interation a macrodef instance.
	 */
	public void execute() {
		if (parallel) {
	...
	protected void doSequentialIteration(String val) {
		...
		/**** ITAI:This line was added in order to pass correctly the parameters to the JSystem task ******/
		getProject().setProperty(param, val);
		/**************************************************************************************************/
	...
	private void doTheTasks() {
		...
		if (keepgoing && (errorCount != 0)) {
			throw new BuildException("Keepgoing execution: " + errorCount
